...
execute as @a[tag=coilgun_replenish_coilgun,tag=hold_coilgun] run item replace entity @s weapon.mainhand with minecraft:knowledge_book{Recipes:["minecraft:crafting_table"],display:{Name:"\"Coilgun\"",Lore:["\"Empty\""]},CustomModelData:2}
...
execute as @a[tag=coilgun_replenish_coilgun,tag=hold_coilgun_loaded,scores={dropped_coilgun=1..}] run item replace entity @s weapon.mainhand with minecraft:knowledge_book{Recipes:["minecraft:crafting_table"],display:{Name:"\"Coilgun\"",Lore:["\"Bullet\""]},CustomModelData:2}
